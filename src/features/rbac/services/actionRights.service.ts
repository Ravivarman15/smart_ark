...
// have run the migration yet. The action evaluator then falls back to the
// catalog defaults (submodule visibility → allow).
...
class ActionRightsService extends BaseService {
  ...
  async upsert(input: ActionRightUpsert, updatedBy?: string): Promise<void> {
    ...
    if (error) throw AppError.fromSupabase(error, "rbac_role_actions.upsert");
  }
  ...
  async upsertMany(rows: ActionRightUpsert[], updatedBy?: string): Promise<void> {
    ...
    const { error } = await this.db
      .from("rbac_role_actions" as never)
      .upsert(payload as never, { onConflict: "role,action_id" });
    if (error) throw AppError.fromSupabase(error, "rbac_role_actions.upsertMany");
  }
  ...
  async resetRole(role: Role | string): Promise<void> {
    const { error } = await this.db
      ...
      .delete()
      .eq("role", role);
    if (error) throw AppError.fromSupabase(error, "rbac_role_actions.reset");
  }
}

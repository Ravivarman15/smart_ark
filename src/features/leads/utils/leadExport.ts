...
/**
 * Load the organization's own logo for the PDF header.
 *
 * Resolves to null when the tenant has no logo, or when the image fails to
 * load — the header then renders text-only. There is deliberately NO default
 * image: the only one available would be another tenant's mark, which is
 * exactly what this file used to stamp on every organization's report.
 */
const loadLogo = (url: string): Promise<HTMLImageElement | null> =>
  new Promise((resolve) => {
    if (!url) return resolve(null);
    const img = new Image();
    // Required for jsPDF to read pixels from a remote logo without tainting.
...
/** Professional PDF: organization logo, generated date, table, summary, footer. */
export const exportCounselorReportPdf = async (rows: CounselorRow[]) => {
  ...
  doc.text(branding.organizationName, margin + 60, y + 18);
  ...
  doc.text(`${branding.organizationName} — Confidential. Generated by Smart ARK.`, margin,
